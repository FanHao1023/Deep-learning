    ...
    def sample(self, images, max_length=15):
        ...
        for i in range(max_length):
            ...
            if self.cell_type == 'rnn':
                ...
                attn, attn_weights_all[:, 1, :, :] = dot_product_attention(h, A)
...
def dot_product_attention(prev_h, A):
    ...
    e = torch.sum(A_flat * prev_h.unsqueeze(-1)/math.sqrt(H), dim=1)
    attn_weights = torch.nn.functional.softmax(e, dim=1)
    attn_temp = attn_weights.view(N, 1, D_a*D_a)
    attn = torch.sum(A_flat * attn_temp, dim=2)
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        N, H = prev_h.shape
        a = prev_h.mm(self.Wh) + x.mm(self.Wx) + self.b
        a = a + torch.mm(attn, self.Wattn)
        i = torch.sigmoid(a[:, 0*H:1*H])
        f = torch.sigmoid(a[:, 1*H:2*H])
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for t in range(T):
            ...
            next_h, next_c = AttentionLSTM.step_forward(self, x[:, t, :], prev_h, prev_c, attn)
            prev_h = next_h
            prev_c = next_c
            hn[:, t, :] = next_h
        ######################################################################
