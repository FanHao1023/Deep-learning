...
from usefuns.solver import Solver
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    loss += self.reg * (torch.sum(W2*W2) + torch.sum(W1*W1))
    
    dout, dw2, db2 = Linear.backward(dS, cache2)
    grads['W2'] = dw2 + self.reg*2*W2
    grads['b2'] = db2
    
    dx, dw1, db1 = Linear_ReLU.backward(dout, cache1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for ii in range(self.num_layers):
        self.params['W%d' % (ii+1)] = weight_scale * torch.randn(all_dims[ii], all_dims[ii+1], device=device, dtype=dtype)
        self.params['b%d' % (ii+1)] = torch.zeros(all_dims[ii+1], device=device, dtype=dtype)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers-1):
        ...
        if self.use_dropout:
            hidden, cache = Dropout.forward(hidden, self.dropout_param)
    ...
    for i in reversed(range(self.num_layers-1)):
        ...
        loss += self.reg * torch.sum(W*W)
        if self.use_dropout:
            dout = Dropout.backward(dout, cache_drop[-1])
            cache_drop.pop()
        dout, dw, db = Linear_ReLU.backward(dout, cache_all[-1])
        cache_all.pop()
...
def create_solver_instance(data_dict, dtype, device):
  ...
  solver = None
  # Replace "pass" statement with your code
  solver = Solver(model, data_dict, update_rule=adam, optim_config={'learning_rate':0.5}, lr_decay=0.96, batch_size=512, num_epochs=50, print_every=100, device='cuda')
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return solver
...
def adam(w, dw, config=None):
  ...
  next_w = w - config['learning_rate'] * m_bias / (torch.sqrt(v_bias) + config['epsilon'])
  #############################################################################
  #                              END OF YOUR CODE                             #
  #############################################################################
