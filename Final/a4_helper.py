...
def train_detector(
    ...
    lr_scheduler = optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=[int(0.5 * max_iters), int(0.75 * max_iters)]
    )
    ...
    for _iter in range(max_iters):
        ...
        if total_loss.item() < best_loss:
            print(f"save {save_path}, loss : {best_loss} --> {total_loss.item()}")
...
def inference_with_detector(
    ...
    for iter_num, test_batch in enumerate(test_loader):
        ...
        if output_dir is not None:
            ...
            detection_visualizer(image, idx_to_class, gt_boxes, pred_boxes)
            """
...
def tensor_to_image(tensor):
    ...
    Args:
        tensor: A torch tensor of shape (3, H, W)
...
# 修改detection_visualizer函數
import matplotlib.patches as patches

def detection_visualizer(image, idx_to_class, gt_boxes=None, pred_boxes=None, points=None):
    """
    ...
    Args:
        ...
        gt_boxes: Ground truth boxes, tensor of shape (N, 5) where each row is [x1, y1, x2, y2, class_id]
        pred_boxes: Predicted boxes, tensor of shape (N, 6) where each row is [x1, y1, x2, y2, class_id, score]
        points: Points to visualize, tensor of shape (P, 2)
    """
    # Convert image tensor to numpy array for visualization
    img = tensor_to_image(image)
    img = img.copy()  # Avoid modifying original image
    ...
    if gt_boxes is not None:
        for box in gt_boxes:
            ...
            ax.add_patch(rect)
            plt.text(x1, y1, class_name, color='green', fontsize=12, verticalalignment='top', bbox={'facecolor': 'white', 'alpha': 0.5})
    ...
    if pred_boxes is not None:
        for box in pred_boxes:
            ...
            ax.add_patch(rect)
            plt.text(x1, y1, f"{class_name}, {score:.2f}", color='red', fontsize=12, verticalalignment='top', bbox={'facecolor': 'white', 'alpha': 0.5})
