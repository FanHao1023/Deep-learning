...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_train_sum = torch.sum(x_train_flat**2, dim=1, keepdim = True)
  x_test_sum = torch.sum(x_test_flat**2, dim=1, keepdim=True).t()
    
  x_dot = torch.matmul(x_train_flat, x_test_flat.t())
  
  dists = x_train_sum + x_test_sum - 2*x_dot
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
    _, idx = torch.topk(-dists[:,i], k)
    y_pred[i],_ = torch.mode(y_train[idx], 0)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
        test_x = x_train_folds[-1]
        test_y = y_train_folds[-1]
        train_x = torch.cat(x_train_folds[:-1], dim=0)
        train_y = torch.cat(y_train_folds[:-1], dim=0)
        classifier = KnnClassifier(train_x, train_y)
        list_of_acc.append(classifier.check_accuracy(test_x, test_y, k))
        x_train_folds.pop(-1)
        y_train_folds.pop(-1)
        y_train_folds.insert(0, test_y)
        x_train_folds.insert(0, test_x)
