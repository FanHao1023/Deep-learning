...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  conv_w1 = nn.init.kaiming_normal_(torch.empty(channel_1, C, kernel_size_1, kernel_size_1, dtype = torch.float, device = 'cuda'))
  conv_w1.requires_grad = True
  conv_b1 = nn.init.zeros_(torch.empty(channel_1, dtype = torch.float, device='cuda'))
  conv_b1.requires_grad = True
  conv_w2 = nn.init.kaiming_normal_(torch.empty(channel_2, channel_1, kernel_size_2, kernel_size_2, dtype = torch.float, device='cuda'))
  conv_w2.requires_grad = True
  conv_b2 = nn.init.zeros_(torch.empty(channel_2, dtype=torch.float, device='cuda'))

  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, channel_2*H*W, dtype=torch.float, device='cuda'))
  fc_w.requires_grad = True

  fc_b = nn.init.zeros_(torch.empty(num_classes, dtype=torch.float, device='cuda'))
...
class ThreeLayerConvNet(nn.Module):
  ...
  def forward(self, x):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    x = F.relu(self.conv2(F.relu(self.cov1(x))))
    x = flatten(x)
    scores = self.fc(x)
    ############################################################################
    #                            END OF YOUR CODE                          
...
class ResNet(nn.Module):
  ...
  def forward(self, x):
    ...
    x = F.avg_pool2d(x, kernel_size=(x_h, x_w))
    x = flatten(x)
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(
      ...
      nn.BatchNorm2d(Cin),
      nn.ReLU(),
      nn.Conv2d(Cin, cout_temp, 1, stride=1, padding=0),

      nn.BatchNorm2d(cout_temp),
      nn.ReLU(),
      nn.Conv2d(cout_temp, cout_temp, 3, stride=2 if downsample else 1, padding=1),
      
      nn.BatchNorm2d(cout_temp),
      nn.ReLU(),
      nn.Conv2d(cout_temp, Cout, 1, stride=1, padding=0),
    )
