...
def nn_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [0.8, 0.9, 1]
  hidden_sizes = [900, 1024, 1200]
  regularization_strengths = [1e-8, 1e-7]
  learning_rate_decays = [0.9, 0.91, 0.92]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
    ...
    for hs in hidden_sizes:
      for reg in regularization_strengths:
        for lrd in learning_rate_decays:
          net = TwoLayerNet(data_dict['X_train'].shape[1], hs, 10)
          stat = net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],                                learning_rate=lr, learning_rate_decay=lrd, reg=reg, num_iters=3000, batch_size=512,                                verbose=False)
          if stat['val_acc_history'][-1] > best_val_acc:
            print(f'lr:{lr} hs:{hs} reg:{reg} lrd:{lrd}')
            best_net = net
            best_stat = stat
