...
def svm_loss_vectorized(W, X, y, reg):
  ...
  margin = torch.clamp(scores - correct_class_score + 1, min=0)
  ...
  mask = margin.clone()
  mask[margin>0] = 1
  mask[torch.arange(num_train), y] = -torch.sum(mask, dim=1).t()
...
def predict_linear_classifier(W, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  y_pred = torch.max(torch.mm(X, W), dim=1).indices
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  return y_pred
...
def svm_get_search_params():
  ...
  regularization_strengths = [0.000001, 0.00001, 0.0001, 0.01, 0.5] 
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  train_acc = torch.sum(train_pred == data_dict['y_train']) / train_pred.shape[-1]
  test_pred = cls.predict(data_dict['X_val'])
  val_acc = torch.sum(test_pred == data_dict['y_val']) / test_pred.shape[-1] 
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
      ...
      loss += -torch.log(exp_correct_class_score / sum(exp_scores))
        
      # Compute the gradient
      for j in range(num_classes):
            if j == y[i]:
                dW[:, y[i]] += (exp_scores[j] / sum(exp_scores)-1)*X[i, :]
            else:
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  # Compute loss
  num_train = X.shape[0]
  loss = -torch.sum(torch.log(probs[torch.arange(num_train), y])) / num_train
